...
//        studentList.forEach(Student::calAge);
        studentList.stream().forEach(Student::calAge);
...
//        --JAVA 8
        ...
        Map<String, Long> rankCount = studentList.stream().collect(Collectors.groupingBy(Student::getRank, Collectors.counting()));
        System.out.println("--Số lượng SV theo từng xếp loại--");
    ...
    public static void searchStudentName() {
        System.out.print("Nhập tên SV bạn muốn tìm kiếm: ");
...
//        --JAVA 8
        ...
        List<Student> searchStudent = studentList.stream()
                .filter(student -> student.getStudentName().toLowerCase().contains(searchKey.toLowerCase()))
                .collect(Collectors.toList());
            if (searchStudent.isEmpty()) {
                System.out.println("Không tồn tại SV tên '"+ searchKey+"'. Vui lòng kiểm tra lại.");
            } else {
                System.out.println("--Danh sách SV tìm kiếm--");
                searchStudent.stream().forEach(Student::displayData);
            }
    }
}
